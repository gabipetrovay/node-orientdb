...
function loadDbSchema(self, callback) {
    ...
    self.loadRecord("#0:0", function(err, result) {
        ...
        self.loadRecord(self.configuration.schemaRecordId, { fetchPlan: "*:-1 index:0" }, function(err, result) {
...
Db.prototype.createClass = function(className, superClassName, callback) {
    ...
    function createClass(className, clusterId, callback) {
    ...
    var classCluster = self.getClusterByName(className);
    if (_.isNull(classCluster)) {

        var cluster = self.getClusterByName("default");
        ...
        self.addDataCluster(clusterOptions, function(err, clusterId) {
            ...
            createClass(className, clusterId, callback);
        });
    } else {
        createClass(className, classCluster.id, callback);
    }
...
Db.prototype.countRecordsInCluster = function(clusterOrIdentifier, callback) {
    callback = callback || EMPTY_FUNCTION;
    
    var self = this;
    var clusterId = null;

    if (_.isString(clusterOrIdentifier)) {
    	var cluster = self.getClusterByName(clusterOrIdentifier);
    	if (_.isNull(cluster)||_.isUndefined(cluster))
    		cluster = self.getClusterByClass(clusterOrIdentifier);
    	clusterId = cluster.id;
    } else if (_.isObject(clusterOrIdentifier) || _.isFunction(clusterOrIdentifier)) {
    	clusterId = clusterOrIdentifier.id;
    } else if (_.isNumber(clusterOrIdentifier)) {
    	clusterId = clusterOrIdentifier;
    }
    
    this.server.send(OperationTypes.DATACLUSTER_COUNT, { clustersId:[clusterId], }, function(err, result) {

        if (err) { return callback(err); }

        result = result || {};
        callback(null, result.clusterCount);
    });
};
...
function makePositionFunc(operation) {
    return function(clusterId, clusterPosition, callback) {
        var data = {};
        if (_.isObject(clusterId)) {
            var ridObj = clusterId;
            data.clusterId = ridObj.clusterId;
            data.clusterPosition = ridObj.clusterPosition;
        } else if (_.isString(clusterId) && (_.isFunction(clusterPosition) || parser.isNullOrUndefined(clusterPosition))) {
            var ridObj = parser.parseRid(clusterId);
            data.clusterId = ridObj.clusterId;
            data.clusterPosition = ridObj.clusterPosition;
        } else {
            data.clusterId = clusterId;
            data.clusterPosition = clusterPosition;
        }
        if (_.isFunction(clusterPosition)) {
            callback = clusterPosition;
            clusterPosition = null;
        }
        callback = callback || EMPTY_FUNCTION;

        if (this.server.manager.serverProtocolVersion < 13) {
            return callback(new Error("positionsHigher and positionsLower are available since OrientDB 1.3.0"));
        }

        this.server.send(operation, data, function(err, result) {
...
Db.prototype.positionsHigher = makePositionFunc(OperationTypes.POSITIONS_HIGHER);

Db.prototype.positionsLower = makePositionFunc(OperationTypes.POSITIONS_LOWER);
